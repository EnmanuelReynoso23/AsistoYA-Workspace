...
class AdvancedAttendanceApp:    def create_settings_tab(self):
        ...
        # Title
        title_label = ttk.Label(main_container, text="Configuración del Sistema", 
                               font=("Arial", 24, "bold"))
        ...
        settings_notebook.add(camera_frame, text="🎥 Cámara")
        
        camera_settings = ttk.LabelFrame(camera_frame, text="Configuración de Cámara", padding=20)
        camera_settings.pack(fill=tk.X, padx=20, pady=20)
        
        # Dropdown for camera selection
        ttk.Label(camera_settings, text="Seleccionar dispositivo:").grid(row=0, column=0, padx=5, pady=5, sticky=tk.W)
        
        # Get camera list
        camera_list = ["Camera 0 (Default)"]
        for i in range(1, 5):  # Check for 5 cameras
            try:
                cap = cv2.VideoCapture(i)
                if cap.isOpened():
        ...
        camera_var = tk.StringVar(value=camera_list[0])
        camera_dropdown = ttk.Combobox(camera_settings, textvariable=camera_var, values=camera_list, width=30)
        camera_dropdown.grid(row=0, column=1, padx=5, pady=5, sticky=tk.W)
        camera_dropdown.bind("<<ComboboxSelected>>", self.change_camera)
        
        # Resolution settings
        ttk.Label(camera_settings, text="Resolución:").grid(row=1, column=0, padx=5, pady=5, sticky=tk.W)
        resolutions = ["640x480", "800x600", "1280x720"]
        resolution_var = tk.StringVar(value=resolutions[0])
        resolution_dropdown = ttk.Combobox(camera_settings, textvariable=resolution_var, values=resolutions, width=30)
        resolution_dropdown.grid(row=1, column=1, padx=5, pady=5, sticky=tk.W)
        
        # Recognition settings tab
        recog_frame = ttk.Frame(settings_notebook)
        settings_notebook.add(recog_frame, text="👤 Reconocimiento")
        
        recognition_settings = ttk.LabelFrame(recog_frame, text="Configuración de Reconocimiento", padding=20)
        recognition_settings.pack(fill=tk.X, padx=20, pady=20)
        
        # Confidence threshold
        ttk.Label(recognition_settings, text="Umbral de confianza:").grid(row=0, column=0, padx=5, pady=5, sticky=tk.W)
        confidence_var = tk.DoubleVar(value=0.6)
        confidence_scale = ttk.Scale(recognition_settings, from_=0.1, to=1.0, variable=confidence_var, length=200)
        confidence_scale.grid(row=0, column=1, padx=5, pady=5, sticky=tk.W)
        ttk.Label(recognition_settings, textvariable=confidence_var).grid(row=0, column=2, padx=5, pady=5, sticky=tk.W)
        
        # Max faces per user
        ttk.Label(recognition_settings, text="Rostros máx. por usuario:").grid(row=1, column=0, padx=5, pady=5, sticky=tk.W)
        faces_var = tk.IntVar(value=3)
        faces_spinbox = ttk.Spinbox(recognition_settings, from_=1, to=10, textvariable=faces_var, width=5)
        faces_spinbox.grid(row=1, column=1, padx=5, pady=5, sticky=tk.W)
        
        # Face training button
        ttk.Button(recognition_settings, text="Re-entrenar Modelo", 
                  command=self.retrain_model, bootstyle="warning").grid(row=2, column=0, columnspan=2, padx=5, pady=15)
        
        # Backup settings tab
        backup_frame = ttk.Frame(settings_notebook)
        settings_notebook.add(backup_frame, text="💾 Respaldo")
        
        backup_settings = ttk.LabelFrame(backup_frame, text="Configuración de Respaldo", padding=20)
        backup_settings.pack(fill=tk.X, padx=20, pady=20)
        
        # Auto backup
        auto_backup_var = tk.BooleanVar(value=True)
        auto_backup_check = ttk.Checkbutton(backup_settings, text="Respaldo automático", variable=auto_backup_var)
        auto_backup_check.grid(row=0, column=0, padx=5, pady=5, sticky=tk.W)
        
        # Backup interval
        ttk.Label(backup_settings, text="Intervalo (horas):").grid(row=1, column=0, padx=5, pady=5, sticky=tk.W)
        interval_var = tk.IntVar(value=24)
        interval_spinbox = ttk.Spinbox(backup_settings, from_=1, to=168, textvariable=interval_var, width=5)
        interval_spinbox.grid(row=1, column=1, padx=5, pady=5, sticky=tk.W)
        
        # Backup location
        ttk.Label(backup_settings, text="Ubicación de respaldo:").grid(row=2, column=0, padx=5, pady=5, sticky=tk.W)
        backup_path_var = tk.StringVar(value="./backup")
        backup_path_entry = ttk.Entry(backup_settings, textvariable=backup_path_var, width=30)
        backup_path_entry.grid(row=2, column=1, padx=5, pady=5, sticky=tk.W)
        ttk.Button(backup_settings, text="Examinar", 
                  command=lambda: self.select_backup_path(backup_path_var)).grid(row=2, column=2, padx=5, pady=5)
        
        # Manual backup button
        ttk.Button(backup_settings, text="Crear Respaldo Ahora", 
                  command=self.create_backup, bootstyle="primary").grid(row=3, column=0, columnspan=3, padx=5, pady=15)
        
        # Apply settings button
        ttk.Button(main_container, text="Guardar Configuración", 
                  command=self.save_settings, bootstyle="success").pack(pady=20)
